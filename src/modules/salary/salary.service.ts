...
export class SalaryService {
  ...
  async export(query) {
    ...
    const filterData = data.map(({ dataValues }) => {
      const {
        user: { phone, chinese_name, employment_date, deposit_bank, english_name, role, gender, id_card_number, bank_account_number } = {} as any
        , commission_rate, basic_salary, settlement_date, class_hour, gross_amount, reward, penalty, total_amount
      } = dataValues
      return {
        "中文名称": chinese_name,
        "英文名称": english_name,
